...
import img1 from "@/assets/courseconnect/IMG 1.jpg";
import img2 from "@/assets/courseconnect/IMG 2.jpg";
import img3 from "@/assets/courseconnect/IMG 3.jpg";
import img4 from "@/assets/courseconnect/IMG 4.jpg";
import img5 from "@/assets/courseconnect/IMG 5.jpg";
import img6 from "@/assets/courseconnect/IMG 6.jpg";
import img7 from "@/assets/courseconnect/IMG 7.jpg";
...
type Slide = {
  src: string;
  group: "Overview" | "Student" | "Admin";
  title: string;
  caption: string;
};

const PROJECT_NAME = "CourseConnect";

const slides: Slide[] = [
  { src: thumb, group: "Overview", title: PROJECT_NAME, caption: "Full stack academic registration platform with automated prerequisite validation, real time schedule conflict detection, and a separate admin portal." },
  { src: img1, group: "Student", title: "Landing / Sign in gate", caption: "New users must sign up first before signing in." },
  { src: img2, group: "Student", title: "Authentication", caption: "Clean email and password login screen." },
  { src: img3, group: "Student", title: "Student registration", caption: "New students register with name, email, and password." },
  { src: img4, group: "Student", title: "Academic dashboard", caption: "Personal overview: registered courses, total credits, completed courses, and current semester, all live from the database." },
  { src: img5, group: "Student", title: "Browse courses", caption: "Smart catalog with real time prerequisite validation and schedule conflict detection. Ineligible courses are blocked with a clear reason." },
  { src: img6, group: "Student", title: "My courses", caption: "Full list of enrolled courses with schedule, instructor, and registration date. Drop directly from this view." },
  { src: img7, group: "Student", title: "Visual timetable", caption: "Every registered course plotted into its actual day and time slot." },
        ...
        <section className="relative w-full bg-background py-10 md:py-14">
          <div className="mx-auto max-w-7xl px-6">
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div className="max-w-2xl">
                ...
                <h2 className="mt-4 font-display text-3xl leading-[0.95] tracking-tight text-foreground md:text-5xl">
                  CourseConnect<span className="text-muted-foreground">.</span>
                </h2>
              </div>
            </div>
            ...
            <div className="mt-6" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
              ...
              <div className="mt-8 grid grid-cols-4 gap-2 sm:grid-cols-6 md:grid-cols-7 lg:grid-cols-7">
                {slides.map((s, i) => (
                  <button
                    key={s.src}
                    onClick={() => go(i)}
